...
These require explicit user approval before implementation (matches `AGENTS.md`):
...
These are sensitive and require caution, but do not require stopping for approval on every touch:
- User data (reviews, visits, photos, personal info) — avoid exposing in logs, errors, or PR summaries.
- RLS policy scope — see `05-security-secrets.md` for details.
